...
<body class="flex min-h-screen">
    ...
    <aside class="w-64 bg-[#0A0C10] border-r border-white/5 flex flex-col fixed top-0 left-0 h-full z-20">
        ...
        <nav class="flex-1 p-4 space-y-1">
            ...
            <a href="/admin/refunds" class="flex items-center gap-3 px-4 py-3 rounded-xl bg-red-500/10 text-red-400 border border-red-500/20 text-sm font-bold">
    ...
    <main class="ml-64 flex-1 p-8 lg:p-10">
        ...
        <div class="mb-8 border-b border-white/5 pb-6 flex flex-col md:flex-row justify-between items-start md:items-end gap-4">
            <div>
                <h1 class="text-3xl font-black text-white tracking-widest uppercase mb-1">Refund</h1>
                <p class="text-gray-500 text-sm">Kelola permintaan pengembalian dana dari pengguna.</p>
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
            <div class="bg-[#12151C] border border-white/5 rounded-2xl p-5 border-l-4 border-l-blue-500">
                <p class="text-gray-500 text-xs font-bold uppercase tracking-widest mb-1">Total Permintaan</p>
                <p class="text-3xl font-black text-white">{{ $refunds->count() }}</p>
            </div>
            <div class="bg-[#12151C] border border-white/5 rounded-2xl p-5 border-l-4 border-l-yellow-500">
                <p class="text-gray-500 text-xs font-bold uppercase tracking-widest mb-1">Menunggu Proses</p>
                <p class="text-3xl font-black text-yellow-400">{{ $refunds->where('status', 'pending')->count() }}</p>
            </div>
            <div class="bg-[#12151C] border border-white/5 rounded-2xl p-5 border-l-4 border-l-green-500">
                <p class="text-gray-500 text-xs font-bold uppercase tracking-widest mb-1">Refund Selesai</p>
                <p class="text-3xl font-black text-green-400">{{ $refunds->where('status', 'approved')->count() }}</p>
            </div>
        </div>
        ...
        <div class="bg-[#12151C] border border-white/5 rounded-2xl overflow-hidden shadow-xl">
            <div class="px-6 py-4 border-b border-white/5 flex items-center gap-3">
                ...
                <h2 class="text-white font-bold uppercase tracking-widest text-sm">Daftar Permintaan Refund</h2>
                <span class="ml-auto text-xs text-gray-500 bg-white/5 px-3 py-1 rounded-full border border-white/10">
                    {{ $refunds->count() }} permintaan
                </span>
            </div>
                ...
                <table class="w-full text-sm">
                    <thead>
                        <tr class="border-b border-white/5">
                            <th class="px-6 py-4 text-left text-xs font-bold text-gray-500 uppercase tracking-widest">ID Refund</th>
                            <th class="px-6 py-4 text-left text-xs font-bold text-gray-500 uppercase tracking-widest">Tanggal / User</th>
                            <th class="px-6 py-4 text-left text-xs font-bold text-gray-500 uppercase tracking-widest">Game</th>
                            <th class="px-6 py-4 text-left text-xs font-bold text-gray-500 uppercase tracking-widest">Alasan</th>
                    ...
                    <tbody class="divide-y divide-[#1a1a1a]">
                        ...
                        <tr class="hover:bg-white/5 transition-colors">
                            <td class="px-6 py-4">
                                <span class="font-mono text-xs text-red-400 bg-red-500/10 border border-red-500/20 px-2 py-1 rounded-lg">#RFD-{{ $r->id }}</span>
                            </td>
                            <td class="px-6 py-4">
                                <div class="text-white text-xs">{{ $r->created_at->format('d M Y, H:i') }}</div>
                                <div class="text-gray-500 text-xs font-mono mt-1">{{ $r->user->username ?? 'Unknown' }}</div>
                            </td>
                            <td class="px-6 py-4 text-white font-semibold">
                                {{ $r->detailTransaksi->game->name ?? 'Game Dihapus' }}
                            </td>
                            ...
                            <td class="px-6 py-4">
                                @if($r->status == 'approved')
                                    <span class="inline-flex items-center gap-1.5 text-xs px-3 py-1.5 bg-green-500/10 text-green-400 border border-green-500/20 rounded-full font-bold">
                                        <span class="w-1.5 h-1.5 bg-green-400 rounded-full"></span> Disetujui
                                    </span>
                                @elseif($r->status == 'rejected')
                                    <span class="inline-flex items-center gap-1.5 text-xs px-3 py-1.5 bg-red-500/10 text-red-400 border border-red-500/20 rounded-full font-bold">
                                        <span class="w-1.5 h-1.5 bg-red-400 rounded-full"></span> Ditolak
                                    </span>
                                @else
                            ...
                            <td class="px-6 py-4 text-center">
                                @if($r->status == 'pending')
                                <form action="/admin/refunds/{{ $r->id }}/process" method="POST" class="inline-flex gap-2">
                                    @csrf
                                    <button type="submit" name="action" value="approve" class="px-3 py-1.5 bg-green-500/20 hover:bg-green-500/40 text-green-400 border border-green-500/30 rounded-lg text-xs font-bold transition-all">Terima & Hapus Game</button>
                                    <button type="submit" name="action" value="reject" class="px-3 py-1.5 bg-red-500/20 hover:bg-red-500/40 text-red-400 border border-red-500/30 rounded-lg text-xs font-bold transition-all">Tolak</button>
                                </form>
                                @else
                                <span class="text-gray-600 text-xs italic">Selesai diproses</span>
                                @endif
                            </td>
                        </tr>
                        @empty
                        <tr>
                            <td colspan="6" class="px-6 py-16 text-center text-gray-600">
                                ...
                                <p class="font-semibold">Belum ada permintaan refund saat ini.</p>
                            </td>
                        </tr>
                        @endforelse
                    </tbody>
