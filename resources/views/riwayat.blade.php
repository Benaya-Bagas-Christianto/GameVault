        ...
        <main class="flex-1 overflow-y-auto hide-scrollbar p-6 lg:p-10 bg-[#050505]">
            <div class="max-w-5xl mx-auto pb-10">
                ...
                <div class="flex flex-wrap items-center gap-4 mb-8 bg-[#0A0C10] p-2 rounded-xl border border-[#1f1f1f] w-fit" id="filterTabs">
                    ...
                    <button data-filter="Refund" class="filter-btn px-4 py-2 text-gray-500 hover:text-white rounded-lg text-xs font-bold transition-colors"><span class="text-red-400 mr-1">●</span> Direfund</button>
                </div>

                {{-- LIST RIWAYAT (TIMELINE) --}}
                <div class="relative timeline-line pl-8 space-y-6">
                    @forelse($transaksi as $t)
                        <div class="relative group trx-card" data-status="{{ $t->refund_state == 'Full' ? 'Refund' : (in_array($t->status, ['Failed', 'Cancelled']) ? 'Failed' : $t->status) }}">
                            {{-- Titik Timeline --}}
                            <div class="absolute -left-[37px] top-6 w-4 h-4 rounded-full border-4 border-[#050505] z-10 {{ $t->refund_state == 'Full' ? 'bg-red-500' : ($t->refund_state == 'Partial' ? 'bg-orange-500' : ($t->status == 'Success' ? 'bg-green-500' : ($t->status == 'Pending' ? 'bg-yellow-500' : 'bg-red-500'))) }}"></div>
                                ...
                                <div class="flex-1">
                                    ...
                                    @if($t->refund_state == 'Full')
                                        <span class="inline-flex items-center gap-1.5 text-[10px] px-2.5 py-1 bg-red-500/10 text-red-400 border border-red-500/20 rounded font-bold uppercase tracking-wider">
                                            <span class="w-1.5 h-1.5 bg-red-400 rounded-full"></span> Direfund
                                        </span>
                                    @elseif($t->refund_state == 'Partial')
                                        <span class="inline-flex items-center gap-1.5 text-[10px] px-2.5 py-1 bg-orange-500/10 text-orange-400 border border-orange-500/20 rounded font-bold uppercase tracking-wider">
                                            <span class="w-1.5 h-1.5 bg-orange-400 rounded-full"></span> Refund Sebagian
                                        </span>
        ...
        function showTransactionDetail(id) {
            ...
            .then(data => {
                if(data.status === 'success') {
                    ...
                    // Status Badge
                    if(trx.refund_state === 'Full') {
                        document.getElementById('modalTrxStatus').innerHTML = `<span class="inline-flex items-center gap-1.5 text-[10px] px-2.5 py-1 bg-red-500/10 text-red-400 border border-red-500/20 rounded font-bold uppercase tracking-wider"><span class="w-1.5 h-1.5 bg-red-400 rounded-full"></span> Direfund</span>`;
                    } else if(trx.refund_state === 'Partial') {
                        document.getElementById('modalTrxStatus').innerHTML = `<span class="inline-flex items-center gap-1.5 text-[10px] px-2.5 py-1 bg-orange-500/10 text-orange-400 border border-orange-500/20 rounded font-bold uppercase tracking-wider"><span class="w-1.5 h-1.5 bg-orange-400 rounded-full"></span> Refund Sebagian</span>`;
                    ...
                    data.items.forEach(item => {
                        ...
                        let badgeHtml = item.is_refunded ? `<span class="px-2 py-0.5 bg-red-500/10 text-red-400 border border-red-500/20 rounded text-[9px] font-black uppercase tracking-wider ml-2">Direfund</span>` : '';
                        let textClass = item.is_refunded ? 'text-gray-600 line-through' : 'text-gray-500';
                        
                        itemsHtml += `
                            <div class="flex items-center gap-4 bg-[#0A0C10] p-3 rounded-xl border border-[#1f1f1f]">
                                <img src="${imgUrl}" class="w-16 h-16 object-cover rounded-lg border border-[#1f1f1f] ${item.is_refunded ? 'grayscale opacity-50' : ''}" onerror="this.src='/assets/no-image.jpg'">
                                <div class="flex-1">
                                    <p class="text-white font-bold text-sm mb-1">${item.name} ${badgeHtml}</p>
                                    <p class="text-xs ${textClass}">Rp ${priceFormatted}</p>
                                </div>
                            </div>
                        `;
                    });
